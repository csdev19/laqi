...
- `Source Serif 4` — all chrome, labels, headings, descriptions (italic for
  descriptions). The serif *is* the UI font; no sans anywhere.
...
- `Reset all to default` clears overrides *and* the scenario; it appears only
...
> **Correcciones pendientes** — ver [revision-vs-decisiones.md](revision-vs-decisiones.md):
