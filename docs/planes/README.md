...
v2.0.0 se implementa en cinco planes. Cada uno produce software que funciona y
se puede testear por sí solo, y se ejecuta en orden.

| # | Plan | Entrega | Estado |
|---|------|---------|--------|
| 1 | [Fundación y servidor de mocks](2026-08-24-01-fundacion-y-servidor.md) | `laqi` corre y sirve mocks desde `laqi.json` o `laqi/`, con validación, hot-reload y las cuatro capas de resolución. Más `laqi migrate`. **13 tareas, ~95 tests.** Auditado: [auditoría](auditoria-plan-01.md). | **Listo para ejecutar** |
| 2 | Control plane y editor web | `/__laqi/api/*`, SSE, y `packages/editor` completo | Por escribir |
...
| 5 | Documentación | `apps/documentation` en Astro + Starlight | Por escribir |
