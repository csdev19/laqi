...
casos de prueba construidos a propósito. Los bugs marcados como *verificados*
se reprodujeron corriendo el servidor, no se dedujeron leyendo.

Este documento es la evidencia que justifica el [ADR-0001 (rewrite)](decisiones/0001-rewrite-v2.md).
...
Cada endpoint declara *varias* respuestas posibles y `codeResponse` elige cuál
...
- El campo `ip` en la config, pensado explícitamente para devs mobile que no
  ...
  de LAN) se quedaba corta — ver [ADR-0007](decisiones/0007-url-publica.md).
...
### A. Fuga de estado entre requests — *verificado*
...
### B. `return` donde iba `continue` — *verificado*
...
### D. Colisión de claves entre archivos — *verificado*
...
resuelve la colisión *dentro* de un archivo, no entre archivos.

### E. `(generate:uid)` nunca se implementó — *verificado*
...
De ahí el [ADR-0001](decisiones/0001-rewrite-v2.md).
