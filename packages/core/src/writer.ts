...
function bodyIsVouchedFor(stored: unknown): { reason: ConflictReason; error: string } | undefined {
  ...
  if (typeof recorded !== 'string') {
    return {
      ...
      error:
        'this body was not generated by laqi — replacing it would discard whatever it holds; pass confirm to go ahead',
    }
  }
  ...
  if (recorded !== bodyHash(entry['body'])) {
    return {
      ...
      error:
        'this body has changed since laqi generated it — replacing it would discard those edits; pass confirm to go ahead',
    }
  }
