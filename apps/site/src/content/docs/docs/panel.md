...
- **Generated from a model or a schema.** The response stores a JSON Schema
  document, and the pane exports types from it in any supported language.
  That document is where the literal unions, optional fields and tuples
  survive — a body cannot carry them.
- **Anything else** — a body you pasted as JSON, one you wrote by hand, an
  endpoint that predates all this — shows the types inferred from the body,
  and says the response carries no schema.

Whatever the schema had to approximate is printed beside the types: a union
narrowed to one member, a type that could not be resolved, a self-reference
that was cut. Copying types without seeing that would be copying a claim laqi
already knows is incomplete.

Because the schema is kept, **Regenerate** uses it rather than guessing the
shape back out of one sample. Guessing is what turned a `[number, number]`
into three numbers and a four-way literal union into a random string — so a
response with no schema is now refused rather than guessed at. Create it from
a model or a JSON Schema first.

What is stored is laqi's rules for generating, not your type definition. It
records how the body was made, not what the body is now: edit the body by
hand and the schema stays as it was. See the
[JSON Schema adapters spec](https://laqi.dev/design/json-schema-adapters/).
